...
namespace Astrana.Core.Data.EntityConfiguration
{
    ...
    {
        public static ModelBuilder ConfigurePhoneNumberType(this ModelBuilder modelBuilder)
        {
            return modelBuilder.ApplyConfiguration(new PhoneNumberTypeConfiguration());
        }
    }

    public class PhoneNumberTypeConfiguration : IEntityTypeConfiguration<PhoneNumberType>
    {
        public void Configure(EntityTypeBuilder<PhoneNumberType> entityTypeBuilder)
        {
            entityTypeBuilder.Property(p => p.Name)
                .IsRequired()
                .HasMaxLength(DomainModelProperties.PhoneNumberType.MaximumNameLength)
                .HasColumnOrder(1);

            entityTypeBuilder.Property(p => p.Code)
                .IsRequired()
                .HasMaxLength(DomainModelProperties.PhoneNumberType.MaximumCodeLength)
                .HasColumnOrder(2);

...
namespace Astrana.Core.Framework.Model.Validation
{
    public static class ValidationUtility
    {
        ...
        public static void ValidateObjectProperties<TModel>(this List<EntityValidationResult> validationResults, TModel instance) where TModel : class
        {
            ...
            foreach (var property in typeof(TModel).GetMembers().Where(m => m.DeclaringType == typeof(TModel) && m.MemberType == MemberTypes.Property).ToList())
            {
                var p = instance.GetType().GetProperties();

                var propertyInfo = GetInstancePropertyInfo(instance, property.Name);
                ...
                else
                {
                    ...
                    {
                        CheckValidationAttribute(validationResults, validationAttribute, propertyValue, property.Name, instance);
                    }
                }
            }
        }

        private static PropertyInfo? GetInstancePropertyInfo<TModel>(TModel instance, string propertyName) where TModel : class
        {
            try
            {
            ...
            {
                return instance.GetType().GetProperties().FirstOrDefault(p => p.Name == propertyName && p.DeclaringType == p.ReflectedType);
            }
        }
        
        private static void CheckValidationAttribute(ICollection<EntityValidationResult> validationResults, Attribute validationAttribute, object? propertyValue, string propertyName, object instance)
        {
            ...
            switch (validationAttribute)
            {
                ...
                case RequiredOnConditionAttribute requiredOnConditionAttribute:
                    {
                        var result = requiredOnConditionAttribute.IsValid(propertyValue); // , new ValidationContext(instance, null, null)

                        if (!result)
                            validationResults.Add(new EntityValidationResult(propertyName, ValidatedEntityType.Property, message: $"A valid value is required. {requiredOnConditionAttribute.ErrorMessage}", outcome: ValidationOutcome.Failed));

                        break;
                    }
                case NotEmptyAttribute notEmptyAttribute:
                    {
                        if (!notEmptyAttribute.IsValid(propertyValue))
                            validationResults.Add(new EntityValidationResult(propertyName, ValidatedEntityType.Property, message: $"A valid value is required. {notEmptyAttribute.ErrorMessage}", outcome: ValidationOutcome.Failed));

                        break;
                    }
                case ValidIdAttribute validIdAttribute:
                    {
                        if (!validIdAttribute.IsValid(propertyValue))
                            validationResults.Add(new EntityValidationResult(propertyName, ValidatedEntityType.Property, message: $"A valid value is required. {validIdAttribute.ErrorMessage}", outcome: ValidationOutcome.Failed));
        ...
        private static object? GetPropertyValue<TModel>(this TModel instance, string propertyName) where TModel : class
        {
            if (instance.GetType().IsAbstract)
                return null;
            ...
            {
                return instance.GetType().GetProperties().FirstOrDefault(p => p.Name == propertyName && p.DeclaringType == p.ReflectedType)?.GetValue(instance, null);
            }
        }
    }
}
